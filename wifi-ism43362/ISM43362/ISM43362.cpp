...
int ISM43362::setCredentialsForAP(const char *ssid, const char *pass, nsapi_security_t security) {
    if (!(_parser.send("AS=0,%s", ssid) && check_response())) {
        debug_if(_ism_debug, "\tISM43362: setting SSID failed\r\n");
        return 1;
    }
    debug_if(_ism_debug, "\tISM43362: setting SSID success\r\n");

    if(security != NSAPI_SECURITY_NONE) {
        if (!(_parser.send("A1=4") && check_response())) {
            debug_if(_ism_debug, "\tISM43362: setting WPA2/WPA wifi failed\r\n");
            return 1;
        }

        if (!(_parser.send("A2=%s", pass) && check_response())) {
            debug_if(_ism_debug, "\tISM43362: setting WPA2 key failed\r\n");
            return 1;
        }
        debug_if(_ism_debug, "\tISM43362: WPA2 key set success\r\n");
    }
...
int ISM43362::startSoftAp() {
    ...
    debug_if(_ism_debug, "\tISM43362: SoftAP started\r\n");
...
int ISM43362::listenForConnection(){
    ...
    while (true) {
        ...
        if (strstr(tmp, "OK") && _conn_status == NSAPI_STATUS_CONNECTING) {
            _parser.flush();
            _conn_status = NSAPI_STATUS_GLOBAL_UP;
            return NSAPI_ERROR_OK;
        }
...
int ISM43362::saveWifiSettings(){
    if (!(_parser.send("Z1") && check_response())) {
        debug_if(_ism_debug, "\tISM43362: saving Wifi settings failed\r\n");
        return 1;
    }
    debug_if(_ism_debug, "\tISM43362: Wifi settings saved\r\n");

    return 0;
}

int ISM43362::connectToWifiIfSaved(){
    char tmp[250];
    if (_parser.send("C0")) {
        while (_parser.recv("%[^\n]\n", tmp)) {
            if (strstr(tmp, "OK")) {
...
/*uint32_t ISM43362::open_pairing_socket(void) {
    char tmp_buffer[250];
    int result;

    if (!(_parser.send("PK=1, 3000") && check_response())) {
        debug_if(_ism_debug, "\ttISM43362: TCP keep alive failed\r\n");
        return 1;
    }
    debug_if(_ism_debug, "\tISM43362: TCP keep alive success\r\n");

    if (!(_parser.send("P1=0") && check_response())) {
        debug_if(_ism_debug, "\ttISM43362: set TCP protocol failed\r\n");
        return 1;
    }
    debug_if(_ism_debug, "\tISM43362: set TCP protocol success\r\n");

    if (!(_parser.send("P2=8080") && check_response())) {
        debug_if(_ism_debug, "\ttISM43362: set TCP port 8080 failed\r\n");
        return 1;
    }
    debug_if(_ism_debug, "\tISM43362: set TCP port 8080 success\r\n");

    if (!(_parser.send("P8=1") && check_response())) {
        debug_if(_ism_debug, "\ttISM43362: set number of backlogs failed\r\n");
        return 1;
    }
    debug_if(_ism_debug, "\tISM43362: set number of backlogs success\r\n");

    tmp_buffer[0]='\0';

    if (!(_parser.send("P5=1"))) {
        debug_if(_ism_debug, "\ttISM43362: TCP server start failed\r\n");
        return 1;
    }
    debug_if(_ism_debug, "\tISM43362: PS5=1 success\r\n");
    result = _parser.recv("%[^\n^\r]\r\n", tmp_buffer);
    debug_if(_ism_debug, "\tISM43362: TCP server 1: %s\r\n", tmp_buffer);
    tmp_buffer[0] = '\0';

    result = _parser.recv("%[^\n^\r]\r\n", tmp_buffer);
    debug_if(_ism_debug, "\tISM43362: TCP server 2: %s\r\n", tmp_buffer);
    tmp_buffer[0] = '\0';

    char *accepted;
    do {
        wait_us(2000);
        result = _parser.send("MR");
        printf("Read result: MR result %d\r\n", result);
        result = _parser.recv("%[^\n^\r]\r\n", tmp_buffer);
        printf("Read result: RECV result %d\r\n", result);

        //if (result >0){
            debug_if(_ism_debug, "\tISM43362: TCP server got: %s\r\n", tmp_buffer);
            accepted = strstr(tmp_buffer, "Accepted");
        //}
...
uint32_t ISM43362::read_data_from_client(void) {
    char data[250];
    char end_of_data[4] = "EOD";

    _parser.send("P0=0");
    
    do {
        _parser.send("R0");
        debug_if(_ism_debug, "\tISM43362: R0 sent\r\n");
        if(!_parser.recv("%[^\n^\r]\r\n", data)) {
            debug_if(_ism_debug, "\tISM43362: Recv data failed\r\n");
            break;
        }
        debug_if(_ism_debug, "\tISM43362: Data: %s\r\n", data);
    } while(true);

    debug_if(_ism_debug, "\tISM43662: Data read end\r\n");
    return 0;
}*/