...
#define ES_WIFI_MAX_TX_PACKET_SIZE                     256
...
public:
    ...
    */
    ...
    virtual int setCredentialsForAP(const char *ssid, const char *pass, nsapi_security_t security);
    virtual int startSoftAp();
    virtual int listenForConnection();
    virtual int saveWifiSettings();
    virtual int connectToWifiIfSaved();

    //virtual uint32_t openPairingSocket();
    //virtual uint32_t readDataFromClient();
